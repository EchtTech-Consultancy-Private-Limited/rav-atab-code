                                 ...
                                 @if(in_array($question['question']->id,$course_doc_uploaded->pluck('doc_unique_id')->all())) 
                                    ...
                                    @if($doc->is_doc_show==0 && $doc->status==0)
                                       <a 
                                        ...
                                        href="{{ url('secretariat-view/verify-doc-level-2' . '/' . dEncrypt($doc->doc_sr_code) .'/' . ($doc->doc_file_name != null ? $doc->doc_file_name : $doc->onsite_doc_file_name) . '/' . dEncrypt($application_id) . '/' . dEncrypt($doc->doc_unique_id).'/'.dEncrypt($course_id)) }}"
                                        class="btn btn-primary btn-sm docBtn m-1">
                                        View</a>
                                        @elseif($doc->status==1)
                                          <a 
                                             title="{{$doc->doc_file_name}}"
                                             href="{{ url('secretariat-accept/verify-doc-level-2' . '/' . dEncrypt($doc->doc_sr_code) .'/' . $doc->doc_file_name . '/' . dEncrypt($application_id) . '/' . dEncrypt($doc->doc_unique_id).'/'.dEncrypt($course_id)) }}"
                                             class="btn btn-success btn-sm docBtn m-1">
                                             Accepted</a>   
                                    @elseif($doc->status==2)
                                    <a 
                                        title="{{$doc->doc_file_name}}"
                                        href="{{ url('secretariat-nc1/verify-doc-level-2' . '/' . dEncrypt($doc->doc_sr_code) .'/' . $doc->doc_file_name . '/' . dEncrypt($application_id) . '/' . dEncrypt($doc->doc_unique_id).'/'.dEncrypt($course_id)) }}"
                                        class="btn btn-danger btn-sm docBtn m-1">
                                        NC1</a>
                                        @elseif($doc->status==3)
                                          <a 
                                             title="{{$doc->doc_file_name}}"
                                             href="{{ url('secretariat-nc2/verify-doc-level-2' . '/' . dEncrypt($doc->doc_sr_code) .'/' . $doc->doc_file_name . '/' . dEncrypt($application_id) . '/' . dEncrypt($doc->doc_unique_id).'/'.dEncrypt($course_id)) }}"
                                             class="btn btn-danger btn-sm docBtn m-1">
                                             NC2</a>
                                        @elseif($doc->status==4)
                                          <a 
                                             title="{{$doc->doc_file_name}}"
                                             href="{{ url('secretariat-nr/verify-doc-level-2' . '/' . dEncrypt($doc->doc_sr_code) .'/' . $doc->doc_file_name . '/' . dEncrypt($application_id) . '/' . dEncrypt($doc->doc_unique_id).'/'.dEncrypt($course_id)) }}"
                                             class="btn btn-danger btn-sm docBtn m-1">
                                              Not Recommended</a>
                                              <!-- admin accept/reject -->
                                             ...
                                             <a 
                                             title="{{$doc->doc_file_name}}"
                                             href="{{ url('secretariat-reject/verify-doc-level-2' . '/' . dEncrypt($doc->doc_sr_code) .'/' . $doc->doc_file_name . '/' . dEncrypt($application_id) . '/' . dEncrypt($doc->doc_unique_id).'/'.dEncrypt($course_id)) }}"
