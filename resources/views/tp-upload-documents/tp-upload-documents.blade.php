                           ...
                           <div class="col-lg-12 col-md-12 col-sm-12">
                              <div class="card project_widget">
                                 ...
                                 <div class="table-responsive">
                                    ...
                                    <input type="hidden" name="doc_sr_code" value="{{$question->code}}">
                                    <input type="hidden" name="doc_unique_id" value="{{$question->id}}">
                                    
                                    

                                    

                                    @if(in_array($question->id,$course_doc_uploaded->pluck('doc_unique_id')->all())) 
                                    @foreach($course_doc_uploaded->filter(function ($item) use ($question) {
                                        return $item['doc_unique_id'] === $question->id;
                                    }) as $doc)
                                        <a target="_blank"
                                        title="{{$doc->doc_file_name}}"
                                        href="{{ url('document-detail' . '/' . $doc->doc_file_name . '/' . $applicationData->id . '/' . $doc->doc_unique_id) }}"
                                        class="btn btn-primary btn-sm docBtn m-1">
                                        View</a>
                                    @endforeach
                                    @else
                                    <input type="file" class="from-control fileup" name="fileup" id="fileup_{{$question->id}}" data-question-id="{{$question->id}}" />
                                    @endif
